...
interface AgenceDocumentsProps {
  agenceId: number;
}

export default function AgenceDocuments({ agenceId }: AgenceDocumentsProps) {
...
const [showNotifications, setShowNotifications] = useState(false);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Charger les contrats actifs de l'agence
      const contratsData = await fetch(`${API_BASE_URL}/agences/contrats?archives=false`, {
      ...
      // Charger les paiements de l'agence
      const paiementsData = await fetch(`${API_BASE_URL}/agences/paiements`, {
        headers: { 'Authorization': `Bearer ${token}` }
      }).then(r => {
        if (r.status === 401) {
          auth.logout();
      ...
      setContrats(contratsData.filter((c: Contrat) => c.statut === 'actif' || c.contrat_statut === 'actif'));
      setPaiements(paiementsData);
      
      console.log('✅ Données chargées:', contratsData.length, 'contrats,', paiementsData.length, 'paiements');
    } catch (error: any) {
  ...
  const handleGenerateQuittance = async () => {
    ...
    try {
      const token = auth.getToken();
      const res = await fetch(`${API_BASE_URL}/agences/pdf/quittance/${selectedPaiement}`, {
        method: 'POST',
        headers: { 
  ...
  const handleGenerateAvis = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/agences/pdf/avis-echeance/${selectedContrat}`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleGenerateCaution = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/agences/pdf/quittance-caution/${selectedContrat}`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 border border-slate-200/60">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-800">Génération de Documents</h1>
            <p className="text-slate-600 mt-1">Créez vos quittances et avis d'échéance en un clic</p>
          </div>
        </div>
                ...
                <div className="space-y-4">
                  <label className="block">
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 border-2 border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white transition-all"
                    >
                  ...
                  {selectedPaiement && (
                    <div className="bg-white p-4 rounded-lg border border-blue-200">
                      <h4 className="font-semibold text-slate-800 mb-2">Détails du paiement</h4>
                      {paiements.filter(p => p.id === selectedPaiement).map(p => (
                        <div key={p.id} className="text-sm text-slate-600 space-y-1">
                ...
                <div className="space-y-4">
                  <label className="block">
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 border-2 border-slate-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white transition-all"
                    >
                  ...
                  {selectedContrat && (
                    <div className="bg-white p-4 rounded-lg border border-purple-200">
                      <h4 className="font-semibold text-slate-800 mb-2">Détails du contrat</h4>
                      {contrats.filter(c => (c.id || c.contrat_id) === selectedContrat).map(c => {
                        const id = c.id || c.contrat_id;
                        const montantLoyer = Number(c.montant_loyer || 0);
                        const charges = Number(c.charges_structurelles || 0) + Number(c.charges_periode || 0);
                        const eau = Number(c.montant_eau || 0);
                        const internet = Number(c.montant_internet || 0);
                        const tva = Number(c.tva || 0);
                        const total = montantLoyer + charges + eau + internet + tva;
                        
                        return (
                          <div key={id} className="text-sm text-slate-600 space-y-1">
                            ...
                            {charges > 0 && <p><span className="font-medium">Charges:</span> {charges.toLocaleString()} FCFA</p>}
                            {eau > 0 && <p><span className="font-medium">Eau:</span> {eau.toLocaleString()} FCFA</p>}
                            {internet > 0 && <p><span className="font-medium">Internet:</span> {internet.toLocaleString()} FCFA</p>}
                            {tva > 0 && <p><span className="font-medium">TVA:</span> {tva.toLocaleString()} FCFA</p>}
                ...
                <div className="space-y-4">
                  <label className="block">
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 border-2 border-slate-300 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white transition-all"
                    >
                      <option value="">-- Sélectionner un contrat --</option>
                      {contrats.map((c) => {
                        const id = c.id || c.contrat_id;
                  ...
                  {selectedContrat && montantCaution && (
                    <div className="bg-white p-4 rounded-lg border border-orange-200">
                      <h4 className="font-semibold text-slate-800 mb-2">Détails de la caution</h4>
                      {contrats.filter(c => (c.id || c.contrat_id) === selectedContrat).map(c => {
                        const id = c.id || c.contrat_id;
                        const montantLoyer = Number(c.montant_loyer || 0);
